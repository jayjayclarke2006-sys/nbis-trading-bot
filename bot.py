...
# ============================================================
# STOCK BEST-SETUP AUTO-TRADER WITH TIME/PRICE EDGE FILTER
# ============================================================
...
def build_opening_range(symbol: str, df5: pd.DataFrame):
    ...
    if session.empty:
        return
    open_min = to_minutes(MARKET_OPEN); or_end_min = to_minutes(OPENING_RANGE_END)
...
def price_zone(symbol, row):
    ...
    if s["OR_SET"] and close > or_high:
        return "above_or"
    if s["OR_SET"] and close < or_low:
        return "below_or"
...
def edge_adjustment(signal, symbol, row):
    ...
    if not best:
        ...
        return signal
    expectancy_r = float(best.get("expectancy_r", 0)); win_rate = float(best.get("win_rate", 0)); trades = int(best.get("trades", 0))
    adj = max(-15, min(15, expectancy_r * 8 + (win_rate - 0.5) * 18))
    signal["score"] = round(signal["score"] + adj, 1)
    signal["edge_note"] = f"edge expR={expectancy_r:.2f}, wr={win_rate:.1%}, n={trades}"
    if trades >= 20 and expectancy_r < -0.08:
        signal["blocked_by_edge"] = True
    return signal


def detect_or_breakout_continuation(symbol: str, df5: pd.DataFrame, row, bias: str):
    s = STATE[symbol]
    if not s["OR_SET"]: return None
    close = float(row["close"]); low = float(row["low"]); high = float(row["high"]); open_ = float(row["open"])
    if close > s["OR_HIGH"] and bias_allows("LONG", bias) and candle_quality(row) and close > open_:
        sl = min(low, s["OR_LOW"]); risk = close - sl
        if risk > 0:
    ...
    if close < s["OR_LOW"] and bias_allows("SHORT", bias) and candle_quality(row) and close < open_:
        sl = max(high, s["OR_HIGH"]); risk = sl - close
        if risk > 0:
...
def detect_or_retest_rejection(symbol: str, df5: pd.DataFrame, row, bias: str):
    s = STATE[symbol]
    if not s["OR_SET"]: return None
    close = float(row["close"]); low = float(row["low"]); high = float(row["high"]); atr = float(row["atr"])
    recent = df5.tail(10)
    bull_broke = (recent["close"] > s["OR_HIGH"]).any(); bear_broke = (recent["close"] < s["OR_LOW"]).any()
...
def detect_trend_pullback(symbol: str, df5: pd.DataFrame, row, bias: str):
    close = float(row["close"]); low = float(row["low"]); high = float(row["high"]); open_ = float(row["open"]); atr = float(row["atr"]); ema20 = float(row["ema20"]); ema50 = float(row["ema50"])
    if bias_allows("LONG", bias):
        ...
        if touched_value and reclaimed and strong_rejection(row, "LONG") and candle_quality(row):
            sl = min(low, ema50 - atr * 0.10); risk = close - sl
            if risk > 0:
    ...
    if bias_allows("SHORT", bias):
        ...
        if touched_value and rejected and strong_rejection(row, "SHORT") and candle_quality(row):
            sl = max(high, ema50 + atr * 0.10); risk = sl - close
            if risk > 0:
...
def detect_liquidity_sweep_reversal(symbol: str, df5: pd.DataFrame, row, bias: str):
    ...
    if len(df5) >= 8:
        swing_low = df5["low"].iloc[i - 8:i].min(); swing_high = df5["high"].iloc[i - 8:i].max()
    else:
        return None
...
def detect_range_rejection(symbol: str, df5: pd.DataFrame, row, bias: str):
    ...
    recent = df5.tail(20)
    range_high = float(recent["high"].max()); range_low = float(recent["low"].min())
    close = float(row["close"]); low = float(row["low"]); high = float(row["high"])
...
def detect_compression_breakout(symbol: str, df5: pd.DataFrame, row, bias: str):
    ...
    recent = df5.tail(12)
    width = float(recent["high"].max() - recent["low"].min()); atr = float(row["atr"])
    high_level = float(recent["high"].max()); low_level = float(recent["low"].min()); close = float(row["close"])
    if atr <= 0: return None
    if width <= atr * 2.4 and close > high_level and bias_allows("LONG", bias):
        sl = float(row["low"]); risk = close - sl
        if risk > 0:
    ...
    if width <= atr * 2.4 and close < low_level and bias_allows("SHORT", bias):
        sl = float(row["high"]); risk = sl - close
        if risk > 0:
...
def detect_ema_reclaim(symbol: str, df5: pd.DataFrame, row, bias: str):
    ...
    prev = df5.iloc[-2]
    close = float(row["close"]); high = float(row["high"]); low = float(row["low"]); ema20 = float(row["ema20"])
...
def get_candidates(symbol: str):
    ...
    candidates = [c for c in candidates if not c.get("blocked_by_edge")]
    if not candidates:
...
def calculate_qty(signal: dict) -> int:
    ...
    if not account:
        return 0
    equity = float(account.get("equity", 0)); buying_power = float(account.get("buying_power", 0))
...
def submit_bracket_order(signal: dict):
    symbol = signal["symbol"]; qty = calculate_qty(signal)
    if qty <= 0:
...
def handle_signal(signal: dict):
    ...
    send_once(
        ...
        f"Edge: {signal.get('edge_note', 'none')}\n"
...
def check_order_updates():
    for symbol in WATCHLIST:
        s = STATE[symbol]; order_id = s["ORDER_ID"]
        if not order_id or order_id == "DRY_RUN":
            continue
...
def end_of_day_summary():
    t = now_ny()
    if minute_of_day(t) < to_minutes(EOD_SUMMARY_TIME): return
    if BOT_STATE["EOD_SENT"]: return

    positions = get_positions(); open_orders = get_open_orders()
    reason_counts = {}; traded_symbols = []

    for symbol in WATCHLIST:
        s = STATE[symbol]; reason = s["LAST_REASON"]
...
def startup_check():
    ...
    send(
        ...
        f"Using edge filter from: {EDGE_PROFILE_FILE}\n"
...
def run():
    ...
    while True:
        try:
            ...
            _BAR_CACHE.clear(); _HTF_CACHE["stamp"] = None; _HTF_CACHE["bias"] = {}
            all_candidates = []
            ...
            if all_candidates and BOT_STATE["TRADES_TODAY"] < MAX_TRADES_PER_DAY:
                ...
                orders_sent = 0; used_symbols = set()
                for signal in ranked:
                    if orders_sent >= MAX_NEW_ORDERS_PER_SCAN: break
                    if BOT_STATE["TRADES_TODAY"] >= MAX_TRADES_PER_DAY: break
                    if signal["symbol"] in used_symbols: continue
                    if STATE[signal["symbol"]]["TRADED_TODAY"]: continue
                    if has_position(signal["symbol"]) or has_open_order(signal["symbol"]): continue
                    ok = handle_signal(signal)
                    if ok:
                        used_symbols.add(signal["symbol"]); orders_sent += 1
