...
# ============================================================
# BTC + GOLD FULL STRATEGY BOT - BULLETPROOF DATA VERSION
# ============================================================
#
# This is the full strategy version with:
# - BTC data fixed properly
# - Proxy routing support
# - Binance -> Coinbase -> TwelveData -> Yahoo fallback
# - Gold TwelveData -> Yahoo fallback
# - Relaxed data validation so working feeds are not rejected
# - Clean UTF-8 emojis
# - Startup alerts, data alerts, heartbeats, entries, SL/TP, BE, trailing
# - NO sniper entries
# - Multi-timeframe SMC scoring
#
# Optional env vars:
# TELEGRAM_TOKEN / TELEGRAM_BOT_TOKEN
# CHAT_ID / TELEGRAM_CHAT_ID
# TWELVEDATA_API_KEY
# PROXY_URL  example format:
...
# Make yfinance use proxy via normal environment variables when PROXY_URL is provided.
if PROXY_URL:
...
DATA_ALERT_COOLDOWN = 1800
SOURCE_SWITCH_ALERT_COOLDOWN = 120
BINANCE_RETRY_SECONDS = 900
DEBUG_MODE = True
...
def alert_source_switch(asset: str, old_source: str, new_source: str):
    state = STATE[asset]
    now = time.time()
    ...
    if now - state["LAST_SOURCE_ALERT"] < SOURCE_SWITCH_ALERT_COOLDOWN:
        return

    send(
        f"â¡ {asset} DATA SWITCHED\n\n"
        f"Old: {old_source or 'NONE'}\n"
        f"New: {new_source}"
    )
    state["LAST_SOURCE_ALERT"] = now


def alert_data_fail(asset: str, feed: str):
    state = STATE[asset]
    now = time.time()

    if now - state["LAST_DATA_FAIL"] < DATA_ALERT_COOLDOWN:
        return

    send(
        f"â ï¸ {asset} DATA ISSUE\n\n"
        f"Feed: {feed}\n"
        f"All feeds failed or returned unusable candles."
    )
...
def validate_ohlcv(df: pd.DataFrame, min_rows: int = 30) -> bool:
    if df is None or df.empty:
        return False
    ...
    try:
        if float(last["close"]) <= 0:
            return False
        if float(last["high"]) < float(last["low"]):
            return False
    except Exception:
        return False
...
def get_binance(symbol: str, interval: str, limit: int = 500) -> pd.DataFrame:
    ...
    for attempt in range(3):
        try:
            ...
            if r.status_code != 200:
                print("BINANCE STATUS ERROR:", r.status_code, r.text[:300])
                time.sleep(1)
                continue
            ...
            if validate_ohlcv(df, min_rows=30):
                return df
...
def get_coinbase(symbol: str, interval: str) -> pd.DataFrame:
    try:
        ...
        if r.status_code != 200:
            print("COINBASE STATUS ERROR:", r.status_code, r.text[:300])
            return pd.DataFrame()
...
def get_twelvedata(symbol: str, interval: str) -> pd.DataFrame:
    try:
        ...
        if r.status_code != 200:
            print("TWELVEDATA STATUS ERROR:", r.status_code, r.text[:300])
            return pd.DataFrame()
...
# ============================================================
# BULLETPROOF DATA ROUTING
# ============================================================
def fetch_from_source(asset: str, interval: str, source: str) -> pd.DataFrame:
    cfg = ASSETS[asset]

    if source == "BINANCE":
        return get_binance(cfg["binance"], interval) if cfg.get("binance") else pd.DataFrame()

    if source == "COINBASE":
        return get_coinbase(cfg["coinbase"], interval) if cfg.get("coinbase") else pd.DataFrame()

    if source == "TWELVEDATA":
        return get_twelvedata(cfg["td"], interval) if cfg.get("td") else pd.DataFrame()

    if source == "YFINANCE":
        return get_yf(cfg["yf"], interval) if cfg.get("yf") else pd.DataFrame()

    return pd.DataFrame()


def source_order(asset: str):
    if asset == "BTC":
        order = ["BINANCE", "COINBASE", "YFINANCE"]
        if TWELVEDATA_API_KEY:
            order = ["BINANCE", "COINBASE", "TWELVEDATA", "YFINANCE"]
        return order

    # Gold: TwelveData if available, then Yahoo
    if TWELVEDATA_API_KEY:
...
def get_klines(asset: str, interval: str):
    state = DATA_STATE[asset]
    active = state["active"]
    now = time.time()

    order = source_order(asset)

    # Try the current working source first.
    if active in order:
        df = fetch_from_source(asset, interval, active)
        if not df.empty:
            state["last_success"] = now
            return df, active

    # For BTC, periodically try to recover back to Binance.
    if asset == "BTC" and active != "BINANCE":
        if now - state["last_binance_retry"] >= BINANCE_RETRY_SECONDS:
            state["last_binance_retry"] = now
            df = fetch_from_source(asset, interval, "BINANCE")
            if not df.empty:
                old = state["active"]
                state["active"] = "BINANCE"
                state["last_success"] = now
                alert_source_switch(asset, old, "BINANCE")
                return df, "BINANCE"
    ...
    for src in order:
        ...
        df = fetch_from_source(asset, interval, src)

        if not df.empty:
            old = state["active"]
            state["active"] = src
            state["last_success"] = now
            alert_source_switch(asset, old, src)
            return df, src
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    # IMPORTANT: this is 40, not 120.
    # Coinbase/Yahoo often return fewer clean rows after processing.
    ...
    # If there are enough rows, use ema200; otherwise use ema50 as a safe replacement.
    if len(out) >= 200:
        out["ema200"] = out["close"].ewm(span=200, adjust=False).mean()
    else:
        out["ema200"] = out["ema50"]
...
def swing_high(df: pd.DataFrame, lookback: int) -> float:
    lookback = min(lookback, max(2, len(df) - 2))
...
def liquidity_sweep_low(df: pd.DataFrame, cfg: dict) -> bool:
    if len(df) < cfg["SWEEP_LOOKBACK"] + 3:
        return False
...
def get_signal(asset: str):
    ...
    if DEBUG_MODE:
        print(asset, "RAW LEN:", len(df1_raw), len(df5_raw), len(df15_raw), "SRC:", src1, src5, src15)
...
def run():
    ...
    send("â BOT STARTING...")
    time.sleep(2)
    proxy_msg = "ON" if PROXY_URL else "OFF"
    td_msg = "ON" if TWELVEDATA_API_KEY else "OFF"
    send(
        f"ð¥ BTC + GOLD FULL STRATEGY BOT LIVE ð¥\n"
