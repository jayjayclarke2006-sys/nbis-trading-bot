...
def alert_source_switch(asset: str, old_source: str, new_source: str):
    ...
    if now - state["last_source_alert"] < 120:
        return

    old_label = old_source if old_source else "NONE"
    send(
...
def source_order(asset: str):
    if asset == "BTC":
        return ["BINANCE", "COINBASE", "YFINANCE"]
    return ["YFINANCE"]
...
def fetch_from_source(asset: str, interval: str, source: str) -> pd.DataFrame:
    ...
    if source == "BINANCE":
        if not cfg.get("binance"):
            return pd.DataFrame()
        return get_binance(cfg["binance"], interval)

    if source == "COINBASE":
        return get_coinbase("BTC-USD", interval)

    if source == "YFINANCE":
        symbol = cfg.get("yf") or "BTC-USD"
        return get_yf(symbol, interval)
...
def get_klines(asset: str, interval: str):
    ...
    # Try the currently working source first so the bot does not keep hammering a dead feed.
    if active in order:
    ...
    # For BTC, periodically try to recover back to Binance because it is the preferred source.
    if asset == "BTC" and active != "BINANCE":
        if now - state["last_binance_retry"] >= SOURCE_RETRY_SECONDS:
